...
export interface VenueListItem {
  ...
  venue_name: string;
  owner_id: string;
  /** TODO(backend): list returns only owner_id; owner_name is mock-enriched. */
  owner_name?: string;
  city: string;
  district: string;
  state: string;
...
/**
 * Front-end permission model. The backend only has a single `admin` role, so
 * every authenticated admin currently gets ALL permissions. Sub-roles
 * (Super/Operations/Support/ReadOnly) are a UI-only abstraction until the
 * backend exposes them. TODO(backend): no sub-role/permission claims.
 */
...
export type AdminRole = 'SUPER_ADMIN' | 'OPERATIONS_ADMIN' | 'SUPPORT_ADMIN' | 'READ_ONLY';

// ─────────────────────────────────────────────────────────────────────────
// Admin — approve/reject result (real backend shape)
// ─────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────
// Admin — dashboard (counts are REAL via /admin/venues; rest ANTICIPATED)
// ─────────────────────────────────────────────────────────────────────────
...
export interface AdminMetrics {
  ...
  approved: number;
  rejected: number;
  suspended: number; // TODO(backend): no SUSPENDED status
  today_registrations: number;
  total_bookings: number; // TODO(backend): mock
  total_revenue: number; // TODO(backend): mock
}
...
export interface AdminDashboardData {
  ...
  registrationsTrend: RegistrationTrendPoint[];
  categories: CategorySlice[];
  recent: VenueListItem[];
}
