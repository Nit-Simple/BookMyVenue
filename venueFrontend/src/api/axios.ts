...
// Namespaced per-app so the venue portal and customer app can share localhost
// without clobbering each other's sessions.
const ACCESS_TOKEN_KEY = 'bmv_venue_access_token';
const REFRESH_TOKEN_KEY = 'bmv_venue_refresh_token';
 ...
 * with realistic latency. To go fully live against the Go/Gin backend, set
 * VITE_USE_MOCK=false — every service call already uses real axios semantics.
 */
async function mockAdapter(config: InternalAxiosRequestConfig) {
  ...
  const latency = 200 + Math.random() * 350;
  await sleep(latency);
...
export const api: AxiosInstance = axios.create({
  ...
  timeout: 15000,
  withCredentials: true, // backend sets an httpOnly refresh_token cookie
});
...
/** Register a callback fired when refresh fails (used by AuthContext for auto-logout). */
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (status === 401 && original && !original._retry && tokenStorage.getRefresh()) {
      ...
      refreshing ??= (async () => {
        try {
          // Refresh always runs against the real backend (snake_case).
...
/** Map an axios error to an HTTP status code (0 when offline / no response). */
